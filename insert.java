...
class insert implements ItemListener,ActionListener,MouseListener{
    String salary[]={"0","80000","60000","40000","35000","25000","15000"};
    String month[]={"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};
    String str[]={"<html><u>Create ID</u></html>","Emp Name","Father Name","Gender","Date of Birth","House Address","Mobile Number","Current Email"};
    int x=80,y=90,x1=220,y1=90;
    JFrame fr,fc,fx;
    JLabel lbl[],l1,l2,l3,l4,l5,l6,l7,lpic,lbrowse;
    ButtonGroup bg;
    JRadioButton r1,r2;
    ...
    public insert(JFrame f1,JFrame f2)
    {
                fx = f1;
		fc = f2;
		...
		fr.setLayout(null);		
		fr.setContentPane(new JLabel(new ImageIcon("images/insert.png")));
		...
		lbrowse.setForeground(Color.pink);
		lbrowse.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		lbrowse.setBounds(505,420,100,30);	
		lbrowse.setFont(new Font("verdana",Font.PLAIN,14));
		lbrowse.addMouseListener(this);
		fr.add(lbrowse);
        
		b1 = new JButton("Save",new ImageIcon("images/save4.png"));
		...
		if(lb==lbrowse){
			try{
				FileDialog fd = new FileDialog(fr,"Choose Photo",FileDialog.LOAD);
				fd.setVisible(true);
				...
				if(path.equals("nullnull"))
				{
					...
					oi = Toolkit.getDefaultToolkit().getImage(path);
					si = oi.getScaledInstance(100,100,Image.SCALE_DEFAULT);
					lpic.setIcon(new ImageIcon(si));
					p = 1;
				}
			}
			catch(Exception e){
		...
		public void actionPerformed(ActionEvent ae)
		{
			if(ae.getSource()==b1)
			{
				...
				{
					JOptionPane.showMessageDialog(fr,"Error! Please Select Photo");
				}	
				else if(txt[5].getText().length()>0)
			{
				...
				else
				{
					...
					try
					{
						...
						PreparedStatement ps = con.prepareStatement("insert into emp_record values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
						ps.setString(1,txt[0].getText());
						ps.setString(2,txt[1].getText());
						ps.setString(3,txt[2].getText());
						ps.setString(4,gen);
						ps.setDate(5,sqlDate);
						ps.setString(6,txt[3].getText());
						ps.setString(7,txt[4].getText());
						ps.setString(8,txt[5].getText());
						ps.setString(9,(String)cb4.getSelectedItem());
						ps.setString(10,(String)cb5.getSelectedItem());
						ps.setString(11,(String)cb6.getSelectedItem());
						ps.setString(12,(String)cb7.getSelectedItem());
						ps.setString(13,(String)cb8.getSelectedItem());
						ps.setString(14,(String)cb9.getSelectedItem());
						ps.setString(15,t6.getText());
						...
						{
							JOptionPane.showMessageDialog(fr,"Record Not Inserted.....");
	        			}
						con.close();
					}
